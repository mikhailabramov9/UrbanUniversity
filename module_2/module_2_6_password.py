...
for i in range(1, a):
    for j in range(1, a):
        if a % (i + j) == 0 and i != j and i < j: # исключил одинаковые числа и повторяющиеся пары
            code.extend([i, j])
        j += 1
    i += 1
code_int = int(''.join(map(str, code))) # склеил лист и преобразовал в число для сверки с ответами
print("Шифр: ", code_int)
